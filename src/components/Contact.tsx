...
const contactCards = [
  { icon: Phone, label: 'Téléphone & WhatsApp', value: '06 05 74 19 15', href: 'tel:+33605741915', iconBg: 'bg-red-600', cardBg: 'bg-red-50 border-red-100' },
  { icon: Mail, label: 'Email', value: 'HPS71.PRO@outlook.fr', href: 'mailto:HPS71.PRO@outlook.fr', iconBg: 'bg-blue-600', cardBg: 'bg-blue-50 border-blue-100' },
  { icon: MapPin, label: 'Zone', value: 'Saône-et-Loire (71)', href: '#', iconBg: 'bg-green-600', cardBg: 'bg-green-50 border-green-100' },
  { icon: Clock, label: 'Disponibilité', value: '24h/24 — 7j/7', href: '#', iconBg: 'bg-amber-500', cardBg: 'bg-amber-50 border-amber-100' },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10" ref={ref}>
          ...
          <p className="animate-on-scroll delay-200 text-gray-500 max-w-lg mx-auto text-sm">
            Réponse en moins de 5 minutes. Urgences : <a href="tel:+33605741915" className="text-red-600 font-bold hover:underline">06 05 74 19 15</a>
          </p>
        </div>

        <div className="grid lg:grid-cols-5 gap-6 lg:gap-8 items-start" ref={ref}>
          {/* Sidebar */}
          <div className="animate-on-scroll lg:col-span-2 space-y-3">
            {contactCards.map((card) => {
              ...
              return (
                ...
                >
                  <div className={`w-10 h-10 rounded-xl ${card.iconBg} flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform shadow-md`}>
                  ...
                  <div>
                    ...
                    <p className="font-black text-navy-900 text-sm mt-0.5">{card.value}</p>
                  </div>
                </a>
              );
            })}
            <a href="https://wa.me/33605741915?text=Bonjour%20PLOMBIER%20SERVICE%2C%20je%20souhaite%20un%20devis%20gratuit."
            ...
            >
              ...
              <div>
                <p className="text-green-100 text-xs font-bold uppercase tracking-wider">WhatsApp Direct</p>
                <p className="text-white font-black text-sm mt-0.5">Chattez maintenant</p>
              </div>
            </a>
          </div>
          ...
          <div className="animate-on-scroll delay-200 lg:col-span-3 bg-white rounded-2xl shadow-card border border-gray-100 overflow-hidden">
            <div className="bg-navy-900 px-6 py-5">
              <h3 className="font-display font-black text-white text-lg">Formulaire de contact</h3>
              <p className="text-navy-200/50 text-xs mt-1">Réponse rapide garantie.</p>
            </div>
            <div className="p-6">
              {status !== 'success' ? (
<form
...
>
  <input type="hidden" name="_subject" value="Nouvelle demande - Formulaire de contact HPS71" />
  <input type="hidden" name="_captcha" value="false" />
  <input type="hidden" name="_template" value="table" />
  <div className="grid sm:grid-cols-2 gap-3.5">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Nom *</label>
                      <input name="name" type="text" value={form.name} onChange={handleChange} placeholder="Jean Dupont" required
                    ...
                    <div>
                      ...
                      <input name="phone" type="tel" value={form.phone} onChange={handleChange} placeholder="06 00 00 00 00" required
                        className="w-full border-2 border-gray-100 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-red-500 bg-gray-50 focus:bg-white transition-colors"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option>Urgence (fuite, dégât d'eau)</option>
                      <option>Débouchage</option>
                      <option>Chauffage / Chaudière</option>
                      <option>Ventilation / VMC</option>
                      <option>Rénovation salle de bain</option>
                      <option>Plomberie cuisine</option>
                      <option>Entretien / Contrat</option>
                      <option>Autre</option>
                    </select>
                  </div>
                  <div>
                  ...
                  >
                    <Send className="w-4 h-4" /> {status === 'submitting' ? 'Envoi en cours...' : 'Envoyer ma demande'}
                  </button>
                </form>
              ) : (
                <div className="py-10 text-center">
                  ...
                  <h3 className="font-display font-black text-xl text-navy-900 mb-2">Envoyé !</h3>
                  <p className="text-gray-500 text-sm">Nous vous contactons rapidement.</p>
